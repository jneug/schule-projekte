...
public class Graph<ContentType> {
    ...
    private List<Vertex<ContentType>> vertices;

    private List<Edge<ContentType>> edges;
    ...
    /**
     * Die Anfrage liefert eine neue Liste aller Knotenobjekte vom Typ
     * List<Vertex>.
     */
    public List<Vertex<ContentType>> getVertices() {
        //Eine neue Liste mit allen Vertex-Objekten erstellen.
        List<Vertex<ContentType>> result = new List<>();
        vertices.toFirst();
    ...
    /**
     * Die Anfrage liefert eine neue Liste aller Kantenobjekte vom Typ
     * List<Edge>.
     */
    public List<Edge<ContentType>> getEdges() {
        //Eine neue Liste mit allen Edge-Objekten erstellen.
        List<Edge<ContentType>> result = new List<>();
        edges.toFirst();
     ...
     */
    public Vertex<ContentType> getVertex( ContentType pContent ) {
        //Vertex-Objekt mit pID als ID suchen.
        Vertex<ContentType> result = null;
        vertices.toFirst();
        while( vertices.hasAccess() && result == null ) {
            if( vertices.getContent().getContent().equals(pContent) ) {
    ...
    public Vertex<ContentType> addVertex( ContentType pContent ) {
        Vertex<ContentType> v = new Vertex<>(pContent);
        addVertex(v);
        return v;
    }
     ...
     */
    public void addVertex( Vertex<ContentType> pVertex ) {
        ...
        if( pVertex != null ) {
            ...
            while( vertices.hasAccess() && freeID ) {
                if( vertices.getContent().getContent().equals(pVertex.getContent()) ) {
                    freeID = false;
                }
                vertices.next();
            }
    ...
    public Edge<ContentType> addEdge( Vertex<ContentType> pFrom, Vertex<ContentType> pTo, double pWeight ) {
        Edge<ContentType> e = new Edge<>(pFrom, pTo, pWeight);
        addEdge(e);
        return e;
    }
     ...
     * passiert nichts.
     */
    public void addEdge( Edge<ContentType> pEdge ) {
        //Pruefen, ob pEdge exisitert.
        if( pEdge != null ) {
            Vertex<ContentType>[] vertexPair = pEdge.getVertices();

            //Einfuegekriterien pruefen.
            if( vertexPair[0] != null && vertexPair[1] != null &&
                this.getVertex(vertexPair[0].getContent()) == vertexPair[0] &&
                this.getVertex(vertexPair[1].getContent()) == vertexPair[1] &&
     ...
     */
    public void removeVertex( Vertex<ContentType> pVertex ) {
        //Inzidente Kanten entfernen.
        edges.toFirst();
        while( edges.hasAccess() ) {
            Vertex<ContentType>[] akt = edges.getContent().getVertices();
     ...
     */
    public void removeEdge( Edge<ContentType> pEdge ) {
     ...
     * zurueckgeliefert.
     */
    public List<Vertex<ContentType>> getNeighbours( Vertex<ContentType> pVertex ) {
        List<Vertex<ContentType>> result = new List<>();
        ...
        while( edges.hasAccess() ) {
            ...
            Vertex<ContentType>[] vertexPair = edges.getContent().getVertices();
     ...
     * zurueckgeliefert.
     */
    public List<Edge<ContentType>> getEdges( Vertex<ContentType> pVertex ) {
        List<Edge<ContentType>> result = new List<>();
     ...
     */
    public Edge<ContentType> getEdge( Vertex<ContentType> pVertex, Vertex<ContentType> pAnotherVertex ) {
        Edge<ContentType> result = null;
